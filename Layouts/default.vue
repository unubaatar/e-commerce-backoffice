<template>
  <v-layout>
    <div style="position: fixed; z-index: 10;" class="d-flex w-100 h-100">
      <v-card
        rounded="lg"
        class="ma-4 pa-4"
        elevation="0"
        style="height: 100%; min-width: 300px;  width: 300px; overflow: scroll"
      >
        <v-list v-for="item in sideBarItems">
          <div class="px-6" style="font-weight: 600; font-size: 14px;">{{ item.name }}</div>
      ...
      <div style="width: 100%">
        <v-card
          class="my-4"
          rounded="lg"
          style="width: 100%; height: 80px; background-color: white"
          elevation="0"
        ></v-card>
      </div>
    </div>

    <div style="background-color: #E8E8E8; height: 120px; z-index: 8; position: fixed; " class="w-100">

    </div>
    <v-main class="main" style="margin-top: 120px; margin-left: 333px; overflow: hidden;">
      <slot />
    </v-main>
  </v-layout>
</template>
